...
vec3 pow3(vec3 n, float p) {
    return vec3(pow(n.x, p), pow(n.y, p), pow(n.z, p));
}

    void main() {
        vec2 pix = fragCoord.xy*resolution;
        vec3 col = texture(screenTexture, floor(pix*0.1)*10.0/resolution).rgb;
        float gray = 0.3 * col.r + 0.59 * col.g + 0.11 * col.b - 0.05;
        int n =  4096;
